...
from morphlaw.morphology import MorphologySpec, MorphologyTemplate
from morphlaw.control.gym_benchmark import ADAPTERS, LOCOMOTION_ADAPTERS
from morphlaw.control.panda_gym_variants import PANDA_MORPH_STOCK_ADAPTERS, PANDA_VARIANT_ADAPTERS
...
def record_from_json(payload: dict[str, object]) -> PairRecord:
    from morphlaw.grammar import RobotGraphSpec
...
def plot_results(
    ...
    fig.suptitle(
        "MorphLaw: Morphology × Law Co-evolution (held-out episodes)",
        fontsize=15,
        fontweight="bold",
    )
    ...
    fig.savefig(output / "morphlaw_comparison.png", dpi=300, bbox_inches="tight")
    fig.savefig(output / "morphlaw_comparison.pdf", bbox_inches="tight")
    plt.close(fig)
...
def main() -> None:
    ...
    parser.add_argument(
        "--output", type=Path, default=None, help="defaults to results/morphlaw_<environment>"
    )
    parser.add_argument(
        "--model",
    ...
    output = args.output or Path(f"results/morphlaw_{args.environment}")
    ...
    for name in args.variants:
        ...
        config = MorphLawConfig(**config_kwargs, knowledge_mode=name)
        variant_archive: dict[tuple, PairRecord] = {}
        law_gen = make_law_generator(
            client, args.environment, adapter, variant_archive, responses, name, reasoning_effort
        )
        morph_gen = make_morph_generator(
            client, args.environment, template, variant_archive, responses, name, reasoning_effort
        )
