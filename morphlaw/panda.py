"""PyBullet-native MorphLaw templates for the five harder Panda-Gym variants.
...
``morphlaw.evaluate`` detects URDF templates via the ``mjcf_template``
flag and forwards ``urdf_path`` plus the remaining parameters as ``gym.make``
keyword arguments, so the standard MorphLaw engine, CEM tuning, knowledge
...
class PandaUrdfTemplate(MorphologyTemplate):
    ...
    def urdf_path(self, spec: MorphologyGenome) -> Path:
        ...
        path = directory / f"morphlaw_{self.env_id}_{digest}.urdf"
        if not path.exists():
...
class PandaReachMovingUrdfTemplate(PandaUrdfTemplate):
    ...
    env_id = "MorphlawPandaReachMoving-v0"
...
class PandaPushIceUrdfTemplate(PandaUrdfTemplate):
    ...
    env_id = "MorphlawPandaPushIce-v0"
    fields = PANDA_ARM_FIELDS + (
...
class PandaSlideGateUrdfTemplate(PandaUrdfTemplate):
    ...
    env_id = "MorphlawPandaSlideGate-v0"
    fields = PANDA_ARM_FIELDS + (
...
class PandaPickDistractorUrdfTemplate(PandaUrdfTemplate):
    ...
    env_id = "MorphlawPandaPickDistractor-v0"
    fields = PANDA_ARM_FIELDS + (
...
class PandaStackNarrowUrdfTemplate(PandaUrdfTemplate):
    ...
    env_id = "MorphlawPandaStackNarrow-v0"
    fields = PANDA_ARM_FIELDS + (
